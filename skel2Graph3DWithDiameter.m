function [A,node,link] = skel2Graph3DWithDiameter(skel,THR, diameter)
%Modified version
...
for i=1:cc2.NumObjects
    ...
    node(i).rank = 0;
    node(i).color = 0;
    node(i).diameter = mean(diam(node(i).idx),'all'); %max(diam(node(i).idx), [],'all');
...
for i=1:cc3.NumObjects
    ...
    node(ni).rank = 0;
    node(ni).color = 0;
    node(ni).diameter = mean(diam(node(ni).idx),'all'); % max(diam(node(ni).idx),[],'all');
	...
	link_cands = intersect(link_cands,cans(:,1));
        ...
        for k=1:length(link_cands)
            ...
            if((ept && length(vox)>THR) || (~ept && i~=n_idx))
                ...
                link(l_idx).rank = 0;
                link(l_idx).color = 0;
                link(l_idx).diameter = mean(diam(link(l_idx).point),'all'); %max(diam(link(l_idx).point),[],'all'); 
