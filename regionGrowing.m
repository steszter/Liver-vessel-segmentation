function [P, J, proportion] = regionGrowing(cIM, initPos, thresVal, maxDist, tfMean, tfFillHoles, tfSimplify)
%Modified version
...
proportion = length(find(J))/sum(sum(sum(cIM>0)));

% text output with final number of vertices
disp(['RegionGrowing Ending:' 'Segmented/All voxels ' num2str(proportion) ' Found ' num2str(length(find(J)))...
